...
def main(
    ...
    with fabric.init_module(empty_init=True):
        model = GPT(config, hyper_conn_type=hyper_conn_type, hyper_conn_n=hyper_conn_n)
...
def fit(
    ...
    with torch.device("meta"):
        meta_model = GPT(model.config, hyper_conn_type=hyper_conn_type, hyper_conn_n=hyper_conn_n)
    ...
    for train_data in train_iterator:
        ...
        if not is_accumulating:
            unclipped_grad_norm = fabric.clip_gradients(model, optimizer, max_norm=train.max_norm)
            optimizer.step()
            optimizer.zero_grad()
            state["step_count"] += 1
        ...
        if state["iter_num"] % log_iter_interval == 0:
            ...
            if unclipped_grad_norm is not None:
                metrics["unclipped_grad_norm"] = unclipped_grad_norm.item() if hasattr(unclipped_grad_norm, "item") else unclipped_grad_norm
