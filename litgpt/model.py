...
from litgpt.hyper_conn import hyper_conn_init_func

class GPT(nn.Module):
    def __init__(self, config: Config, hyper_conn_type="none", hyper_conn_n=4) -> None:
        super().__init__()
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                h=nn.ModuleList(Block(config, block_idx, init_hc) for block_idx in range(config.n_layer)),
    ...
    def forward(
        ...
        if hasattr(self, "expand_stream") and self.expand_stream is not None:
        ...
        if hasattr(self, "reduce_stream") and self.reduce_stream is not None:
...
class AttnBranch(nn.Module):
    def __init__(self, norm_1, attn, post_attn_norm):
        super().__init__()
        self.norm_1 = norm_1
        self.attn = attn
        self.post_attn_norm = post_attn_norm

    def forward(self, x, cos, sin, mask=None, input_pos=None, input_pos_maxp1=None):
        x = self.norm_1(x)
        x = self.attn(x, cos, sin, mask, input_pos, input_pos_maxp1)
        return self.post_attn_norm(x)

class MLPBranch(nn.Module):
    def __init__(self, norm_2, mlp, post_mlp_norm):
        super().__init__()
        self.norm_2 = norm_2
        self.mlp = mlp
...
class Block(nn.Module):
    def __init__(
        ...
        self.branch_attn = AttnBranch(self.norm_1, self.attn, self.post_attention_norm)
        self.branch_mlp = MLPBranch(self.norm_2, self.mlp, self.post_mlp_norm)
    ...
    def forward(
        ...
        # ==========================================
        if self.init_hc is not None:
            # cos, sin 等参数会通过你代码里的 *branch_args 完美传递给 AttnBranch
            x = self.hc_attn(x, cos, sin, mask, input_pos, input_pos_maxp1)
            x = self.hc_mlp(x)
            return x

        # ==========================================
        # 原始 LitGPT 的传统残差逻辑保持不变
        # ==========================================
